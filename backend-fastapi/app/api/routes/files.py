...
import logging
from app.core.dependencies import get_current_user, get_file_service
...
async def upload_file(
    ...
    """
    try:
        file_info = await file_service.save_upload_file(file, current_user["uid"])
        return file_info
...
async def analyze_file_by_id(
    ...
    """
    try:
        result = await file_service.analyze_file(file_id)
...
async def process_file(
    ...
    try:
        ...
        if request.processing_type == "extract_text":
            ...
            text = await file_service.extract_text_from_file(file_info.filename)
